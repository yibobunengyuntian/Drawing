...
ShapeArrowsDownItem::ShapeArrowsDownItem(const QPoint &startPoint, const QPoint &endPoint)
    :RectOpItem(startPoint, endPoint)
{
    drawing(startPoint, endPoint);
}

void ShapeArrowsDownItem::updatePath()
{
    m_path.clear();
    ...
    m_path.moveTo(QPointF(right - width / 4.0, top));
    m_path.lineTo(QPointF(left + width / 4.0, top));
    m_path.lineTo(QPointF(left + width / 4.0, top + height / 2.0));
    m_path.lineTo(QPointF(left, top + height / 2.0));
    m_path.lineTo(QPointF(left + width / 2.0, bottom));
    m_path.lineTo(QPointF(right, top + height / 2.0));
    m_path.lineTo(QPointF(right - width / 4.0, top + height / 2.0));

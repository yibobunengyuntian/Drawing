...
ShapeArrowsRightItem::ShapeArrowsRightItem(const QPoint &startPoint, const QPoint &endPoint)
    :RectOpItem(startPoint, endPoint)
{
    drawing(startPoint, endPoint);
}

void ShapeArrowsRightItem::updatePath()
{
    m_path.clear();
    ...
    m_path.lineTo(QPointF(left, top + height / 4.0));
    m_path.lineTo(QPointF(left, bottom - height / 4.0));
